...
@WebServlet("/ser_vdr/BackOneUpdateSerVdrServlet.do")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class BackOneUpdateSerVdrServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Update".equals(action)) { // 查詢所有廠商 來自listAllEmp.jsp的請求
			...
			try {
				...
				String url = "/backend/ser/serVdr/update_SerVdr_input.jsp";
