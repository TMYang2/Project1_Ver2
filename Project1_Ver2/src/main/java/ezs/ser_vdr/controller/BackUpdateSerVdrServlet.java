...
@WebServlet("/ser_vdr/BackUpdateSerVdrServlet.do")
@MultipartConfig(fileSizeThreshold = 1024 * 1024, maxFileSize = 5 * 1024 * 1024, maxRequestSize = 5 * 5 * 1024 * 1024)
public class BackUpdateSerVdrServlet extends HttpServlet {
...
//				}
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/backend/ser/serVdr/update_SerVdr_input.jsp");
				...
				serVdrSvc.updateSerVdr(vdrStatus, vdrName, vdrTel, vdrVatno, vdrCounty, vdrDist, vdrAddr, vdrOpen, vdrIntro, vdrPic, vdrRevCount, vdrRevScore, vdrID);

				/*************************** 3.Edit完成,準備轉交(Send the Success view) *************/
				req.setAttribute("serVdrVO", serVdrVO);// 資料庫update成功後,正確的的empVO物件,存入req
				String url = "/backend/ser/serVdr/listOneSerVdr.jsp";// 傳到前端
...
//				RequestDispatcher failureView = req.getRequestDispatcher("/frontend/ser_vdr/update_SerVdr_input.jsp");
//				failureView.forward(req, res);
//			}
		}
	}
